...
function SearchResult({ user, onItemClick }) {
  ...
  return (
    <div onClick={() => onItemClick(user)} style={{ padding: "2px", cursor: "pointer", transition: "background-color 0.2s ease-in-out"}}>
      {fname} {lname}
    </div>
  );
}
...
function SearchResults({ results, onItemClick }) {
  return (
    <div>
      {results.map((result) => (
        <SearchResult key={result.uid} user={result} onItemClick={onItemClick} />
      ))}
    </div>
  );
}
...
function Searcher() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      if (query.trim() !== "" && !(/^\d+$/.test(query.trim())) && !(/^\S+@\S+\.\S+$/.test(query.trim()))) {
  ...
  return (
    ...
    >
      ...
      <input type="text" value={query} onChange={handleQueryChange} style={{}} />
